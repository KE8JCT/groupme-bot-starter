...
class Bot {
    ...
    static checkMessage(message) {
        ...
        const checkTL = /(((((^W)|(^(D)))|(^(Is)))|(^(Are)))|((((^w)|(^(d)))|(^(is)))|(^(are)))).*(?=(\?)$)/;
        // Check if the GroupMe message has content and if the regex pattern is true
        if (messageText && checkTL.test(messageText)) {
            // Check is successful, return a message!
            return 'Ask your team leader';
        }
        
        const varTL =  /(^Can|^can).*(?=(\?)$)/;
        //It got a little bit long there
        // Check if the GroupMe message has content and if the regex pattern is true
        if (messageText && varTL.test(messageText)) {
            // Check is successful, return a message!
            return 'Ask your team leader';
        }
        ...
        if (messageText && askThomas.test(messageText)) {
            // Check is successful, return a message!
            return textArray[randomNumber]; //fix this
        }
